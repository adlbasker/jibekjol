    ...
    <div class="col-md-6 mb-2">
      <ul class="nav nav-tabs">
        @foreach ($languages as $language)
          <li class="nav-item">
            <a class="nav-link @if ($language->slug == $productLang->lang) active @endif" href="/{{ $language->slug }}/admin/products/{{ $product->id }}/edit">{{ $language->title }}</a>
          </li>
        @endforeach
        ...
        <div class="card mb-3">
          ...
          <div class="card-body">
            <div class="mb-3">
              ...
              <input type="text" class="form-control" id="title" name="title" minlength="5" maxlength="255" value="{{ old('title', $productLang->title) }}" required>
            </div>
            <div class="mb-3">
              <label for="slug" class="form-label">URI адрес</label>
              <input type="text" class="form-control" id="slug" name="slug" minlength="2" maxlength="255" value="{{ old('slug', $productLang->slug) }}">
            </div>
            <div class="mb-3">
              <label for="meta_title" class="form-label">Мета название (краткий заголовок, который отображается в результатах поиска)</label>
              <input type="text" class="form-control" id="meta_title" name="meta_title" maxlength="255" value="{{ old('meta_title', $productLang->meta_title) }}">
            </div>
            <div class="mb-3">
              ...
              <input type="text" class="form-control" id="meta_description" name="meta_description" maxlength="255" value="{{ old('meta_description', $productLang->meta_description) }}">
            </div>
            <div class="mb-3">
            ...
            <div class="mb-3">
              ...
              <div>
                @include('components.bootstrap-5-editor', ['attribute' => 'description', 'content' => old('description', $productLang->description)])
              </div>
            </div>
            <div class="mb-3">
              <label for="characteristic" class="form-label">Характеристика</label>
              <input type="text" class="form-control" id="characteristic" name="characteristic" minlength="2" maxlength="80" value="{{ old('characteristic', $productLang->characteristic) }}">
            </div>
            <div class="row">
              <div class="col-md-6">
                <div class="mb-3">
                  ...
                  @if(array_key_exists($i, $images))
                    <div class="col-md-6">
                      ...
                      <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="alt_description_{{ $i }}" name="alt_descriptions[]" value="{{ $images[$i][$lang.'_description'] ?? null }}">
                        <input type="hidden" class="form-control" name="initial_alt_descriptions[]" value="{{ $images[$i][$lang.'_description'] ?? null }}">
                      </div>
                    </div>
                  @else
                    <div class="col-md-6">
          ...
          <div class="card-body">
            <div class="mb-3 mt-4">
              ...
              <select id="lang" name="lang" class="form-select" required>
                @foreach($languages as $language)
                  <option value="{{ $language->slug }}" @if ($language->slug == $productLang->lang) selected @endif>{{ $language->title }}</option>
                @endforeach
              </select>
            </div>
            <div class="mb-3">
            ...
            <div class="card mb-3">
              <div class="card-body py-2" style="max-height: 250px; overflow-y: auto;">
                <?php foreach ($categories as $category) : ?>
                  <div class="form-check">
                    <input class="form-check-input" type="radio" name="category_id" id="category{{ $category->id }}" value="{{ $category->id }}" <?php if ($productLang->category_id == $category->id) echo "checked"; ?> required>
            ...
            <div class="card">
              <div class="card-body py-2" style="max-height: 150px; overflow-y: auto;">
                @foreach($modes as $mode)
                  ...
                  <div class="form-check">
                    <input class="form-check-input" type="checkbox" name="modes_id[]" id="mode{{ $mode->id }}" value="{{ $mode->id }}" <?php if ($product->modes->contains($mode->id)) echo "checked"; ?>>
