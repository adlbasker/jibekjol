          ...
          <form action="{{ route('sections.store', $lang) }}" method="post">
            ...
            <div id="keyValue">
              @for ($i = 0; $i < 3; $i++)
                <div class="row mb-3">
                  <div class="col-md-4">
                    <label for="key_{{ $i }}" class="form-label">Название {{ $i }}</label>
                    <input type="text" class="form-control" id="key_{{ $i }}" name="data[key][]" maxlength="255" value="{{ old('data.key.' . ($i-1)) }}">
                  </div>
                  <div class="col-md-8">
                    <label for="value_{{ $i }}" class="form-label">Значение {{ $i }}. Разделитель /</label>
                    <input type="text" class="form-control" id="value_{{ $i }}" name="data[value][]" maxlength="255" value="{{ old('data.value.' . ($i-1)) }}">
                  </div>
                </div>
              @endfor
            </div>
...
<script>
  function addInput() {
    ...
    const nextId = container.querySelectorAll('.row').length + 1;
