        ...
        <div class="card-body">
          <form action="{{ route('categories.update', [$lang, $category->id]) }}" method="post" enctype="multipart/form-data">
            ...
            <div class="mb-3">
              ...
              <input type="text" class="form-control" id="title" name="title" minlength="2" maxlength="80" value="{{ old('title', $category->title) }}" required>
            </div>
            <div class="mb-3">
              <label for="title_extra" class="form-label">Название дополнительное</label>
              <input type="text" class="form-control" id="title_extra" name="title_extra" minlength="2" maxlength="80" value="{{ old('title_extra', $category->title_extra) }}">
            </div>
            <div class="mb-3">
              <label for="slug" class="form-label">Slug</label>
              <input type="text" class="form-control" id="slug" name="slug" minlength="2" maxlength="80" value="{{ old('slug', $category->slug) }}">
            </div>
            <div class="mb-3">
              <label for="category_id" class="form-label">Категории</label>
              <select id="category_id" name="category_id" class="form-select">
                <option value=""></option>
                <?php $traverse = function ($nodes, $prefix = null) use (&$traverse, $category, $__env) { ?>
            ...
            <div class="mb-3">
              ...
              <input type="text" class="form-control" id="sort_id" name="sort_id" maxlength="5" value="{{ old('sort_id', $category->sort_id) }}">
            </div>
            <div class="mb-3">
              <label for="meta_title" class="form-label">Мета заголовок</label>
              <input type="text" class="form-control" id="meta_title" name="meta_title" maxlength="255" value="{{ old('meta_title', $category->meta_title) }}">
            </div>
            <div class="mb-3">
              <label for="meta_description" class="form-label">Мета описание</label>
              <input type="text" class="form-control" id="meta_description" name="meta_description" maxlength="255" value="{{ old('meta_description', $category->meta_description) }}">
            </div>
            <div class="mb-3">
              ...
              <select id="lang" name="lang" class="form-select" required>
                ...
                @foreach($languages as $language)
                  <option value="{{ $language->slug }}" {{ $category->lang == $language->slug ? 'selected' : '' }}>{{ $language->title }}</option>
                @endforeach
              </select>
            </div>
            <div class="mb-3">
              ...
              @foreach(trans('statuses.data') as $num => $status)
                <div class="form-check">
                  <input class="form-check-input" type="radio" name="status" id="status{{ $num }}" value="{{ $num }}" {{ $num == $category->status ? 'checked' : '' }}>
