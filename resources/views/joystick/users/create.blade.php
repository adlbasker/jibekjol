          ...
          <div class="card-body">
            <div class="mb-3">
              ...
              <select id="region_id" name="region_id" class="form-select">
                <option value=""></option>
                <?php $traverse = function ($nodes, $prefix = null) use (&$traverse, $user, $__env) { ?>$__env) { ?>
            ...
            <div class="mb-3">
              ...
              @foreach(trans('data.gender') as $key => $value)
                <div class="form-check form-check-inline">
                  <input class="form-check-input" type="radio" name="gender" id="gender{{ $key }}" value="{{ $key }}" @if($user->gender == $key) checked @endif>
            ...
            <div class="mb-3">
              <div class="form-check">
                <input class="form-check-input" type="checkbox" id="is_debtor" name="is_debtor" @if ($user->is_debtor) checked @endif disabled>
                <label class="form-check-label" for="is_debtor">Должник</label>
              </div>
            </div>
            <div class="mb-3">
              <label for="debt_sum" class="form-label">Сумма долга</label>
              <input type="number" class="form-control" id="debt_sum" name="debt_sum" value="{{ old('debt_sum', $user->debt_sum ?? 0) }}">
            </div>
            <div class="mb-3">
              <label for="bonus" class="form-label">Бонус</label>
              <input type="number" class="form-control" id="bonus" name="bonus" value="{{ old('bonus', $user->bonus ?? 0) }}">
            </div>
            <div class="mb-3">
              <label for="discount" class="form-label">Скидка</label>
              <input type="number" class="form-control" id="discount" name="discount" value="{{ old('discount', $user->discount ?? 0) }}">
            </div>
          </div>
        </div>
      </div>
