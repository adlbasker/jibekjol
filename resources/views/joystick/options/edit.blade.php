    ...
    <div class="col-md-6 mb-2">
      <ul class="nav nav-tabs">
        @foreach ($languages as $language)
          <li class="nav-item">
            <a class="nav-link @if ($language->slug == $lang) active @endif" href="/{{ $language->slug }}/admin/options/{{ $option->id }}/edit">{{ $language->title }}</a>
          </li>
        @endforeach
      </ul>
    </div>
        ...
        <div class="card-body">
          <form action="/{{ $lang }}/admin/options/{{ $option->id }}" method="post">
            ...
            <div class="mb-3">
              ...
              <input type="text" class="form-control" id="slug" name="slug" maxlength="80" value="{{ old('slug', $option->slug) }}">
            </div>
            <div class="mb-3">
              <label for="sort_id" class="form-label">Номер</label>
              <input type="text" class="form-control" id="sort_id" name="sort_id" value="{{ old('sort_id', $option->sort_id) }}">
            </div>
            <div class="mb-3">
              <label for="data" class="form-label">Группа</label>
              <?php $data = json_decode($option->data, true); ?>
              <input type="text" class="form-control" id="data" name="data" value="{{ old('data', $data[$lang]['data']) }}">
            </div>
            <div class="mb-3">
