...
class ProductController extends Controller
{
    ...
    public function store(Request $request, $lang)
    {
        ...
        if ($request->hasFile('images')) {
            $images = $this->saveImages($request, $lang, $dirName);
        ...
        return redirect($request->lang.'/admin/products')->with('status', __('statuses.entry_added'));
    }
    ...
    public function update(Request $request, $lang, $id)
    {
        ...
        // Adding new images
        if ($request->hasFile('images')) {
            if (! file_exists(public_path('img/products/'.$product->path)) OR empty(public_path($product->path))) {
                $dirName = $product->company->id.'/'.time();
                File::makeDirectory(public_path('img/products/'.$dirName));
                $product->path = $dirName;
            }

            $images = $this->uploadImages($request, $lang, $dirName, $images, $product);
            $introImage = current($images)['present_image'];
        }
        else {
            foreach ($request->initial_alt_descriptions as $key => $ia_description) {
                if ($request->alt_descriptions[$key] !== $ia_description) {
                    $images[$key][$lang.'_description'] = $request->alt_descriptions[$key];
                }
            }
        }

        // Change directory for new category
        if ($product->company_id != $request->company_id AND file_exists(public_path('img/products/'.$product->path))) {
        ...
        return redirect($lang.'/admin/products')->with('status', __('statuses.entry_changed'));
    }

    public function saveImages($request, $lang, $dirName)
    {
        $order = 1;
        $images = [];
        ...
        foreach ($request->file('images') as $key => $image)
        {
            ...
            $this->resizeOptimalImage($image, 400, 360, '/img/products/'.$dirName.'/present-'.$imageName, 90);

            // Storing original images
            // $image->storeAs('/img/products/'.$dirName, $imageName);
            $this->resizeOptimalImage($image, 1000, 900, '/img/products/'.$dirName.'/'.$imageName, 85);
    ...
    public function uploadImages($request, $lang, $dirName, $images = [], $product)
    {
    ...
    public function destroyComment($id)
    {
        ...
        return redirect($lang.'/admin/products/'.$comment->parent_id.'/comments')->with('status', __('statuses.entry_deleted'));
    }
}